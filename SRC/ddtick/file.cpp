...
File::File(string _filename) : filename(_filename), valid(true) {
  if (!(*this).ok()) {
    if(!(*this).match_ics()) {
      throw std::runtime_error("File is not OK " + (*this).filename);
    }
  }
}
...
int File::CRC() {
  ...
  return filecrc32((*this).filename.c_str());
}

File File::Copy(string dest) {
  if(!(*this).valid)
    throw runtime_error("File is not valid");
  ...
  sfile = open((*this).filename.c_str(), O_RDONLY);
  if(sfile == -1) {
    throw runtime_error("Error opening source file");
  }
  dfile = open(dest.c_str(), O_WRONLY | O_CREAT, 0666);
  if(dfile == -1) {
    throw runtime_error("Error opening dest file " + dest);
  }
    
  while((BytesRead = read(sfile, buffer, 1024)) > 0) {
    if(write(dfile, buffer, BytesRead) != BytesRead) {
      close(dfile);
      close(sfile);
      throw runtime_error("Error copying file");
    } 
  }
...
File File::Move(string dest) {
  if(!(*this).valid)
    throw runtime_error("File is not valid");
  
  File nf = (*this).Copy(dest);
...
bool File::match_ics() {
  ...
  string dirname = (*this).filename.substr(0, pos);
  string filename = (*this).filename.substr(pos+1);

  FindFileFunctor f(filename);
  ScanDir<FindFileFunctor> s(dirname, false, "", f);
  s.Scan();

  if(f.found) {
    (*this).filename = f.found_filename;
    return (*this).ok();
  }
...
int File::memcrc32(const char *str, int size, int initcrc) {
  int crc = initcrc;
  
  if(str) {
    for (; size; str++, size--) {
      crc = (*this).crc32tab()[((int) crc ^ (*str)) & 0xff] ^ ((crc >> 8) & 0x00ffffffUL);
    }
  }
  return crc;
}

int File::filecrc32(const char *filename) {
  FILE *fd;
  ...
  buffer = (unsigned char*) malloc(CRC_BUFFER_SIZE);
  ...
  while (1) {
    ...
    if( got )
      crc = (*this).memcrc32((char*)buffer, got, crc);
