...
static word timeout2;		/* general timeout in secs   */
...
static char txfname[FNAMELEN], rxfname[FNAMELEN];	/* fname of current files   *//* OLSEN */
...
static int process_packets(int, char *);
static void finfo(void);
...
boolean hydra_devsend(char *dev, byte * data, word len)
{
	...
	if (chattimer > 0L && !strcmp(devtxdev, "CON")
	    && txstate == HTX_REND)
...
/*---------------------------------------------------------------------------*/
boolean hydra_devfunc(char *dev, void (*func) (byte *, word))
{
	int i;
...
/*---------------------------------------------------------------------------*/
static word crc16block(byte * buf, word len)
{
	word crc;
...
/*---------------------------------------------------------------------------*/
static dword crc32block(byte * buf, word len)
{
	dword crc;
...
/*---------------------------------------------------------------------------*/
static byte *put_binbyte(byte * p, byte c)
{
	byte n;
...
/*---------------------------------------------------------------------------*/
static void txpkt(word len, int type)
{
	byte *in, *out;
	word c, n;
	boolean crc32 = false;
	...
	default:
		...
		if (txoptions & HOPT_HIGHBIT) {
			if ((txoptions & HOPT_CTLCHRS)
			    && (txoptions & HOPT_CANUUE))
				format = HCHR_UUEPKT;
	...
	if (loglevel == 0) {
		...
		message(0,
			" -> PKT (format='%c'  type='%c'  crc=%d  len=%d)",
			format, type, crc32 ? 32 : 16, len - 1);

		switch (type) {
		case HPKT_START:
			message(0, "    <autostr>START");
			break;
		case HPKT_INIT:
			s1 = ((char *) txbufin) +
			    ((int) strlen((char *) txbufin)) + 1;
			s2 = s1 + ((int) strlen(s1)) + 1;
			s3 = s2 + ((int) strlen(s2)) + 1;
			s4 = s3 + ((int) strlen(s3)) + 1;
			message(0,
				"    INIT (appinfo='%s'  can='%s'  want='%s'  options='%s'  pktprefix='%s')",
		...
		case HPKT_FINFOACK:
			if (rxfd >= 0) {
				...
				s1 = "EOB";
			message(0,
				"    FINFOACK (pos=%ld %s  rxstate=%d  rxfd=%d)",
				rxpos, s1, rxstate, rxfd);
			break;
		case HPKT_DATA:
			message(0, "    DATA (ofs=%ld  len=%d)",
				intell(h_long1(txbufin)), len - 5);
			break;
		case HPKT_DATAACK:
			message(0, "    DATAACK (ofs=%ld)",
				intell(h_long1(txbufin)));
			break;
		case HPKT_RPOS:
			message(0,
				"    RPOS (pos=%ld%s  blklen=%ld  syncid=%ld)",
				rxpos, rxpos < 0L ? " SKIP" : "",
				intell(h_long2(txbufin)), rxsyncid);
			break;
		case HPKT_EOF:
			message(0, "    EOF (ofs=%ld%s)",
				txpos, txpos < 0L ? " SKIP" : "");
			break;
		case HPKT_EOFACK:
			message(0, "    EOFACK");
		...
		case HPKT_DEVDATA:
			message(0,
				"    DEVDATA (id=%ld  dev='%s'  len=%u)",
				devtxid, devtxdev, devtxlen);
			break;
		case HPKT_DEVDACK:
			message(0, "    DEVDACK (id=%ld)",
...
static int rxpkt(void)
{
	...
	while ((c = com_getbyte()) >= 0) {
		...
		if (rxoptions & HOPT_HIGHCTL)
			n &= 0x7f;
		if (n != H_DLE && (((rxoptions & HOPT_XONXOFF)
				    && (n == XON || n == XOFF))
				   || ((rxoptions & HOPT_CTLCHRS)
				       && (n < 32 || n == 127))))
			continue;
		...
		if (rxdle || c == H_DLE) {
			...
			case HCHR_PKTEND:
					...
					if (n >= 2) {
						...
						*q++ =
						...
						if (n == 3) {
							...
							*q++ =
							    (byte) ((h_uudec(p[1]) << 4) | (h_uudec(p[2]) >> 2));
						}
					}
					break;
				...
				default:	/* This'd mean internal fluke */
					...
					if (loglevel == 0) {
						message(0,
							" <- <PKTEND> (pktformat='%c' dec=%d hex=%02x) ??",
							rxpktformat,
							rxpktformat,
							rxpktformat);
					}
				...
				if (rxpktformat != HCHR_HEXPKT
				    && (rxoptions & HOPT_CRC32)) {
					if (rxpktlen < 5
					    || rxpktlen > H_BUFLEN) {
						c = H_NOPKT;
						break;
					}
					n = h_crc32test(crc32block
							(rxbuf, rxpktlen));
				...
				if (n) {
					...
					if (loglevel == 0) {
						...
						case HPKT_FINFOACK:
							message(0,
								"    FINFOACK (pos=%ld  txstate=%d  txfd=%d)",
								intell
								(h_long1
								 (rxbuf)),
								txstate,
						...
						case HPKT_RPOS:
							message(0,
								"    RPOS (pos=%ld%s  blklen=%u->%ld  syncid=%ld%s  txstate=%d  txfd=%d)",
								intell
								(h_long1
								 (rxbuf)),
								intell
						...
						default:
							message(0,
								"    Unkown pkttype %d (txstate=%d  rxstate=%d)",
								(int)
								rxbuf
								[rxpktlen],
								txstate,
	...
	if (h_timer_running(braindead) && h_timer_expired(braindead)) {
		...
		if (loglevel == 0)
			message(0,
				" <- BrainDead (timer=%08lx  time=%08lx)",
				braindead, time(NULL));
#endif
		return (H_BRAINTIME);
	}
	...
	if (h_timer_running(devtxtimer) && h_timer_expired(devtxtimer)) {
		...
		if (loglevel == 0)
			message(0,
				" <- DevTxTimer (timer=%08lx  time=%08lx)",
...
{
	long pos = xmit ? txpos : rxpos, fsize = xmit ? txfsize : rxfsize;

	long start = xmit ? txstart : rxstart;
	long elapsed = time(NULL) - start;
	long cps;

	if (elapsed && pos >= 1024L)
		cps = pos / elapsed;
	else
		cps = 0;

	hydra_gotoxy(36, xmit ? 1 : 2);
	hydra_printf("                                         ");
	hydra_gotoxy(36, xmit ? 1 : 2);
	if (pos >= fsize)
		hydra_printf("%ld/%ld [%ld CPS] (EOF)", pos, fsize, cps);
	else {
		int left;
		char *p = "";
		if (!cps)
		...
		if (xmit) {
			if (txstate == HTX_DATAACK)
				p = "ACK ";
			else if (txstate == HTX_XWAIT)
				p = "WAIT ";
		}
		hydra_printf("%ld/%ld [%ld CPS] (%s%d:%02d min left)",
			     pos, fsize, cps, p, left / 60, left % 60);
	}
/*        hydra_clreol(); */
}				/*hydra_status() */
...
static void hydra_pct(boolean xmit)
{
	long offset = xmit ? txoffset : rxoffset,
	    ...
	    start = xmit ? txstart : rxstart, elapsed, bytes, cps, pct;
...
void hydra_init(dword want_options)
{
	...
	for (i = 0; i < 256; i++) {
		...
		for (j = 8; j > 0; j--) {
			if (crc16 & 1)
				crc16 = (crc16 >> 1) ^ h_crc16poly;
			else
				crc16 >>= 1;
			if (crc32 & 1)
				crc32 = (crc32 >> 1) ^ h_crc32poly;
			else
				crc32 >>= 1;
		}
		crc16tab[i] = crc16;
	...
	timeout2 = (word) (40960L / cur_speed);
	if (timeout2 < H_MINTIMER)
		timeout2 = H_MINTIMER;
	else if (timeout2 > H_MAXTIMER)
		timeout2 = H_MAXTIMER;
...

	{
		...
		sprintf(kalabuf,
			"%s v%s - COPYRIGHT (C) 1991-1993 A.G.Lentz, LENTZ SOFTWARE-DEVELOPMENT\n",
			PRGNAME, HVERSION);
		waddstr(logwin, kalabuf);
		waddstr(trwin, "Sending  :\nReceiving:\n");
	}
...
int hydra(char *txpathname, char *txalias)
{
	...
	if (txpathname) {
		...
		if ((txfd = dos_sopen(txpathname, 0)) < 0) {
			message(3, "-HSEND: Unable to open %s",
				txpathname);
			return (XFER_SKIP);
		}
	   ...
	   /*----------------------------------------------------------------*/
		...
		case HTD_DATA:
			if (txstate > HTX_RINIT) {
				...
				txpkt(((int) sizeof(long)) + H_FLAGLEN +
				      1 + devtxlen, HPKT_DEVDATA);
				devtxtimer = h_timer_set(timeout2);
				devtxstate = HTD_DACK;
			}
			break;
	   ...
	   /*----------------------------------------------------------------*/
		...
		case HTX_INIT:
			...
			sprintf(p, "%08lx%s,%s %s",
				H_REVSTAMP, PRGNAME, HVERSION, HC_OS);
			...
			txoptions = rxoptions;
			txtimer = h_timer_set(timeout2 / 2);
			txstate = HTX_INITACK;
			break;
...
/*                             hydra_clreol(); */
			...
			txpkt(((int) strlen((char *) txbufin)) + 1,
			      HPKT_FINFO);
			txtimer =
			    h_timer_set(txretries ? timeout2 /
					2 : timeout2);
	   ...
	   /*----------------------------------------------------------------*/
		process_packets(pkttype, txpathname);
	} while (txstate);
...
static int process_packets(int pkttype, char *txpathname)
{
	char *p;
	char *q;
	int res;
	...
	while (txstate && (pkttype = rxpkt()) != H_NOPKT) {
		...
		case H_TXTIME:
			...
			message(0, "-HSEND: Timeout - Retry %u",
				txretries);
		...
		case H_DEVTXTIME:
			...
			message(0, "-HDEVTX: Timeout - Retry %u",
				devtxretries);
		...
		case HPKT_INIT:
			if (rxstate == HRX_INIT) {
				...
				if (rxoptions < (options & HNEC_OPTIONS)) {
					message(6,
						"!HYDRA: Incompatible on this link");
					txstate = HTX_DONE;
					res = XFER_ABORT;
					break;
				}
				...
				sscanf(p, "%08lx%08lx", &rxwindow,
				       &txwindow);
				if (rxwindow < 0L)
					rxwindow = 0L;
				if (hydra_rxwindow &&
				    (!rxwindow
				     || hydra_rxwindow < rxwindow))
					rxwindow = hydra_rxwindow;
				if (txwindow < 0L)
					txwindow = 0L;
				if (hydra_txwindow &&
				    (!txwindow
				     || hydra_txwindow < txwindow))
				...
				if (!batchesdone) {
					...
					if ((q = strchr(p, ',')) != NULL)
						*q = '/';
					message(0,
						"*HYDRA: Other's App.Info '%s'",
						p);
					put_flags((char *) rxbuf,
						  h_flags, rxoptions);
					message(1,
						"*HYDRA: Using link options '%s'",
						rxbuf);
					if (txwindow || rxwindow)
						message(0,
							"*HYDRA: Window tx=%ld rx=%ld",
							txwindow,
							rxwindow);
				}

				chattimer =
				    (rxoptions & HOPT_DEVICE) ? 0L : -2L;
			...
			if (txstate == HTX_FINFO
				...
				if (!txfname[0]) {
					...
					if (txpos >= 0L) {
						...
						if (txpos > 0L) {
							message(1,
								"+HSEND: Transmitting from offset %ld (%d min. to go)",
								txpos,
								(int)
								((txfsize -
								  txoffset)
						...
						if (txpos == -1L) {
							hydra_gotoxy
							    (29, 1);
							hydra_printf
							    ("They already have file");
							message(1,
								"+HSEND: They already have %s",
								txfname);
							if (mailer)
								resultlog
								    (true,
				...
				if (intell(h_long1(rxbuf)) != rxpos
					...
					if (!h_timer_running(rxtimer)
						...
						if (i <= 64)
							...
							i = 1024;
						message(0,
							"-HRECV: Bad pkt at %ld - Retry %u (newblklen=%u)",
							rxpos,
							rxretries, i);
						h_long1(txbufin) =
						    intell(rxpos);
						...
						txpkt(3 * ((int)
						      ...
						      HPKT_RPOS);
						rxtimer =
						    h_timer_set(timeout2);
					}
				} else {
					braindead =
					    h_timer_set(H_BRAINDEAD);
		...
		case HPKT_RPOS:
			if (txstate == HTX_XDATA
				...
				if (intell(h_long3(rxbuf)) != txsyncid) {
					...
					hydra_status(true);
					message(0,
						"+HSEND: Resending from offset %ld (newblklen=%u)",
						txpos, txblklen);
					if (dos_seek
					    (txfd, txpos, SEEK_SET) < 0L) {
						message(6,
							"!HSEND: File seek error");
						dos_close(txfd);
						txfd = -1;
				...
				if (intell(h_long1(rxbuf)) < 0L) {
					...
					if (!h_timer_running(rxtimer)
						...
						if (i <= 64)
							...
							i = 1024;
						message(0,
							"-HRECV: Bad EOF at %ld - Retry %u (newblklen=%u)",
							rxpos,
							rxretries, i);
						h_long1(txbufin) =
						    intell(rxpos);
		...
		case HTX_XDATA:
			if (rxstate && hdxlink) {
				...
				hydra_devsend("MSG", (byte *) hdxmsg, (int)
					      strlen(hdxmsg));
...
static void new_file(void)
{
	...
	if (!rxpathname) {
		if (single_done) {
			...
			message(1,
				"+HRECV: Skipping additional files (file %s)",
				rxfname);
			rxpos = -2L;
		} else {
			hydra_gotoxy(29, 2);
	...
	if (fexist(rxpathname)) {
		if ((rxfd = dos_sopen(rxpathname, 0)) < 0) {
			message(6, "!HRECV: Unable to re-open %s",
				rxpathname);
			rxpos = -2L;
		}
	...
	message(2, "+HRECV: %s (%ldb), %d min.", rxfname, rxfsize,
	...
	if (rxpos > 0L) {
		...
		message(1, "+HRECV: Resuming from offset %ld (%d min. to go)",
			rxpos, (int) ((rxfsize - rxoffset) * 10L /
				cur_speed + 27L) / 54L);
	}
	rxstate = HRX_DATA;
}
